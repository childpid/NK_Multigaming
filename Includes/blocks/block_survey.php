...
function affich_block_survey($blok){
    ...
    if ($file != 'Survey'){
        ...
        while (list($voteid, $optiontext) = mysql_fetch_array($sql2)){
            ...
            $blok['content'] .= '<label class="radio"><input type="radio" class="checkbox" name="voteID" value="' . $voteid . '" />&nbsp;' . $optiontext . '</label>'."\n";
        }

        $blok['content'] .= '</p><p style="text-align: center"><input type="hidden" name="poll_id" value="' . $poll_id . '" />'."\n"
                                    . '<br /><input type="submit" class="btn" value="' . _TOVOTE . '" />&nbsp;'
                                    . '<input type="button" class="btn" value="' . _RESULT . '" onclick="document.location=\'index.php?file=Survey&amp;op=affich_res&amp;poll_id=' . $poll_id . '\'" />'."\n"
                                    . '<br /></br><a href="index.php?file=Survey" class="btn btn-success">' . _OTHERPOLL . '</a></p></div></form>'."\n";
        }
	$blok['content'] = '<div id="blocksurvey">' . $blok['content'] . '</div>';
    return $blok;
}
