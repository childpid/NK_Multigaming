...
function affich_block_rss($blok){
	...
	if ($handle = @fopen($rssHost, "r")) {
		...
		if($titreactu == 'Oui') $blok['content'] .= '<h3>'.$title.'</h3>';
	...
	$blok['content'] = '<div id="blockrss">' . $blok['content'] . '</div>';
    return $blok;
}
...
function edit_block_rss($bid){
	...
	list($url, $titreactu, $nbr) = explode('|', $content);
...
function modif_advanced_rss($data){
	$data['content'] = $data['content'] . '|' . $data['titreactu'] . '|' .$data['nbr'];
	return $data;
}
?>