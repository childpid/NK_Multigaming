...
function affich_block_roster($blok){
    ...
    while (list($pseudo, $mail, $country) = mysql_fetch_array($sql)){
        ...
        $blok['content'] .= '<tr><td class="rosterflag"><img src="images/flags/' . $country . '" alt="" title="' . $pays . '" /></td>'."\n"
								. '<td class="rosternick"><a href="index.php?file=Team&amp;op=detail&amp;autor=' . urlencode($pseudo) . '"><b>' . $nick_team . '</b></a></td>'."\n"
								. '<td class="rostermail"><a href="mailto:' . $mail . '"><img style="border: 0;" src="' . $img . '" alt="" title="' . $mail . '" /></a></td></tr>'."\n";
	} 

    $blok['content'] .= '</table>'."\n";
	$blok['content'] = '<div id="blockroster">' . $blok['content'] . '</div>';
    return $blok;
} 
