...
function affich_block_menu($blok){
    ...
    $blok['content'] = '<div id="blockmenu">' . $blok['content'] . '</div>';
	return $blok;
}
    ...
    for($i=0; $i<$size; $i++){
        ...
        if ($nivuser >= $nivo){
            if ($url <> '' && $title <> '' && $blank == 0)
                $screen .= '<li><a href="' . $url . '" title="' . $comment . '">' . $title . '</a></li>';

            if ($url <> '' && $title <> '' && $blank == 1)
                $screen .= '<li><a href="' . $url . '" title="' . $comment . '" onclick="window.open(this.href); return false;">' . $title . '</a></li>';

            if ($url == '' && $title <> '' && $comment == '')
                $screen .= '<li class="nav-header">' . $title . '</li>';
        }
    }
    $screen .= '</ul>';
    return $screen;
