...
if ($active == 3 || $active == 4)
{
    ...
    {
        echo "<form class=\"form-inline\" method=\"post\" onsubmit=\"maFonctionAjax(this.textbox_auteur.value,this.textbox_texte.value, this.code.value); return false;\" action=\"\" ><div style=\"text-align: center;\">\n";

        if (!$user)
        {
            echo "<input id=\"textbox_auteur\" class=\"input-small\" type=\"text\" name=\"auteur\" placeholder=\"" . _NICKNAME . "\" onclick=\"if(this.placeholder=='" . _NICKNAME . "'){this.value=''}\" />\n";
        }
        else
        {
        ...
        echo "<input id=\"textbox_texte\" class=\"input-xxlarge\" type=\"text\" name=\"texte\" size=\"50\" placeholder=\"" . _YOURMESS . "\"  onclick=\"if(this.value=='" . _YOURMESS . "'){this.value=''}\" /><br />\n";

		if ($captcha == 1) create_captcha(3);
		else echo "<input id=\"code\" type=\"hidden\" value=\"0\" />\n";

	echo "<br /><input type=\"submit\" class=\"btn\" value=\"" . _SEND . "\" />\n"
	...
	. "<img src=\"modules/textbox/images/smiley.png\" /></a></div><div style=\"text-align: center;\"><br /><a class=\"btn btn-primary\" href=\"index.php?file=Textbox\">" . _SEEARCHIVES . "</a></div></form>\n";
    }
}
else
{
    if ($visiteur >= nivo_mod("Textbox"))
    {
        ...
        echo "<input id=\"textbox_texte\" class=\"input-xxlarge\" type=\"text\" name=\"texte\" placeholder=\"" . _YOURMESS . "\"  style=\"width:90%;\" onclick=\"if(this.value=='" . _YOURMESS . "'){this.value=''}\" /><br /><table>\n";

	if ($captcha == 1) create_captcha(2);
	else echo "<input id=\"code\" type=\"hidden\" value=\"0\" />\n";

	echo "</table><br /><input type=\"submit\" value=\"" . _SEND . "\"/><br /><br />\n"
	. "<a href=\"#\" onclick=\"javascript:window.open('index.php?file=Textbox&amp;nuked_nude=index&amp;op=smilies&amp;textarea=textbox_texte','smilies','toolbar=0,location=0,directories=0,status=0,scrollbars=1,resizable=0,copyhistory=0,menuBar=0,width=200,height=350,top=100,left=470');return(false)\">\n"
